...
def consume_item(item, t_inventory):
    if check_item(item, t_inventory):
        if t_inventory[item] <= 0:
            print(item + "의 수량이 부족하여 사용할 수 없습니다.")            
        else:
            inventory[item] -= 1
...
def use_item(inventory):
    while True:
        print_itemMenu()
        option = int(input("메뉴 번호를 고르시오.)"))

        if option == 0:
            print("종료합니다.")
            break
        elif option == 1:
            nem_item = input("아이템을 입력하세요.")
            amount = int(input("수량을 입력하세요."))
            #TODOLIST 1
            #함수인자 추가하는 기능 구현시 호출 부분 수정 필
            add_item(nem_item, amount, inventory)
        elif option == 2:
...
while True:
    ...
    if option == 0:
        ...
        temp_name = input("선택할 캐릭텅의 이름을 입력해주세요.")
